...
void CodeGen::visitGlobal(Global *gVar)
{
    gVar->type_->accept(this);
    visitIdent(gVar->ident_);
    symbols.insert(Symbol(currid, currtype, symbols.numvars()+5));
}
...
void CodeGen::visitProg(Prog *prog)
{
    code.begin_prog();
    code.prolog(symbols);
    symbols["putn"]->numargs() = 1;
    symbols["puts"]->numargs() = 1;
    symbols["getnum"]->numargs() = 0;
    symbols["exit"]->numargs() = 0;
...
void CodeGen::visitFun(Fun *fun)
{
    ...
    int i = 0;
    for(ListDecl::iterator i = fun->listdecl_->begin(); i!=fun->listdecl_->end(); i++)
    {
        (*i)->accept(this);
        symbols[fun_name]->addType(currtype);
...
void CodeGen::visitSIfThen(SIfThen *sifthen)
{
   ...
   //code.add(i_jr); identical to while except without jump back to init cond
   //code.add(looploc - (code.pos() - 1)); // offset to looploc
    code.at(patchloc) = code.pos() - (patchloc - 1);
}
...
void CodeGen::visitSIfElse(SIfElse *sifelse)
{
    ...
    code.add(I_JR);
    code.add(0);
    int startelse = code.pos() - 1;
    code.at(patchloc) = code.pos() - (patchloc - 1);
    sifelse->stm_2->accept(this);
    code.at(startelse) = code.pos() - (startelse - 1);
}
...
void CodeGen::visitSFor2(SFor2 *sfor2)
{
    ...
    sfor2->stm_->accept(this); // body.
    sfor2->exp_2->accept(this);
    code.add(I_JR); //jump to loop init expression check
    code.add(looploc - (code.pos() - 1)); // offset to looploc
    code.at(patchloc) = code.pos() - (patchloc - 1);
}
...
{
    code.add(I_CALL);
    int arStart = code.pos();
    code.add(0);
    int forStart = code.pos();
    code.add(0);
    code.add(I_JR);
    int arEnd =  code.pos();
    code.add(0);

    code.at(forStart) = code.pos();
    int existing = symbols.numvars();
    code.add(I_PROC);
    int patchloc = code.pos();
    code.add(0);
    code.add(code.pos()+1);
    symbols.enter();
    int vars = symbols.numvars();
    sfor3->exp_1->accept(this);
    symbols.insert(Symbol(currid, currtype, 3+existing+symbols.numvars()));
    code.add(I_VARIABLE);
    ...
    code.add(1);
    code.add(I_VALUE);
    code.at(arStart) = symbols.levelof(currid);
    int looploc = code.pos();
    sfor3->exp_2->accept(this);
    code.add(I_JR_IF_FALSE);
    code.add(0);
    int patchloop = code.pos() - 1;
    sfor3->stm_->accept(this);
    sfor3->exp_3->accept(this);
    code.add(I_JR);
    code.add(looploc - (code.pos() - 1));
    code.at(patchloop) = code.pos() - (patchloop - 1);
    code.at(patchloc) = symbols.numvars() - vars + existing;
    symbols.leave();
    code.add(I_ENDPROC);
    code.at(arEnd) = code.pos() - (arEnd - 1);
}


void CodeGen::visitSRUntil(SRUntil *sruntil)
{
    int looploc = code.pos();
    sruntil->stm_->accept(this);
    sruntil->exp_->accept(this);
    code.add(I_JR_IF_TRUE);
    code.add(looploc - (code.pos() - 1));
