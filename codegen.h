...
class FuncArgC : public std::logic_error
{
public:
    FuncArgC(const std::string &name)
        : logic_error("Function " + name + "has invalid number of arguments.")
        {}
};

class FuncArgT : public std::logic_error
{
public:
    FuncArgT(const std::string &name)
        : logic_error("Function " + name + "has wrong argument type(s)")
        {}
};
