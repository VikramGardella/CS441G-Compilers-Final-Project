...
SRUntil::SRUntil(const SRUntil & other)
{
  stm_ = other.stm_->clone();
  exp_ = other.exp_->clone();
...
SRUntil &SRUntil::operator=(const SRUntil & other)
{
  SRUntil tmp(other);
  swap(tmp);
  return *this;
...
void SRUntil::swap(SRUntil & other)
{
  std::swap(stm_, other.stm_);
  std::swap(exp_, other.exp_);
...
void SRUntil::accept(Visitor *v)
{
  v->visitSRUntil(this);
}

SRUntil *SRUntil::clone() const
{
  return new SRUntil(*this);
}
